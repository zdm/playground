...
export default class extends App {
    ...
    async _testStorage () {
        ...
        const path = "/1/test.txt";

        res = await this.storage.uploadFile( path, new File( { "buffer": "1234567890" } ) );
        console.log( res + "" );
        if ( !res.ok ) process.exit();

        console.log( await this.storage.glob( "**", { "cwd": "/1" } ) );

        res = await this.storage.deleteFile( path );
        console.log( res + "" );
        if ( !res.ok ) process.exit();
